...
function unique(list) {
  var uh = {};
  for(v in list) { if(list.hasOwnProperty(v)) {
    uh[list[v]]++;
  } }
  // return the unique keys of uh
...
var id3 = function(_s, target, features, parentFeatureText, maxDepth, depth){
  var targets = unique(_s.pluck(target));
  if (depth==maxDepth || targets.length == 1){
    console.log("end node! "+targets[0]);
    return {type:"result", val: targets[0], name: parentFeatureText+" : "+targets[0],alias:targets[0]+randomTag() }; 
  }
  if(features.length == 0){
    console.log("returning the most dominant feature!!!");
    var topTarget = mostCommon(targets);
    return {type:"result", val: topTarget, name: parentFeatureText+" : "+topTarget, alias: topTarget+randomTag()};
  }
  var bestFeature = maxGain(_s,target,features);
  var remainingFeatures = _.without(features,bestFeature);
  var possibleValues = unique(_s.pluck(bestFeature));
  console.log("node for "+bestFeature);
  var node = {name: bestFeature,alias: bestFeature+randomTag()};
  node.type = "feature";
  node.name = parentFeatureText;
  node.children = _.map(possibleValues,function(v){
    ...
    child_node.child =  id3(_newS, target, remainingFeatures, "=="+v);
    return child_node;*/
    return id3(_newS, target, remainingFeatures, bestFeature+"="+v, maxDepth, depth+1);
  });
  return node;
}
...
var gain = function(_s,target,feature){
  var attrVals = unique(_s.pluck(feature));
  ...
  var sumOfEntropies = sum(entropies);
...
var maxGain = function(_s,target,features){
  ...
  var m = -1e100;
  var mFeat;
  for(f in features) { if(features.hasOwnProperty(f)) {
  var g = gain(_s, target, features[f]);
  if(g > m) {
    m = g;
    mFeat = features[f];
  }
