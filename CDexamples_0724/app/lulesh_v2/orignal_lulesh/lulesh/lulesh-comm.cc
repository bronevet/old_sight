...
//int cd::MPI_Isend_internal(void* buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request)
int MPI_Isend_internal(void* buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request)
{  
   int src;
...
//int cd::MPI_Irecv_internal(void* buf, int count, MPI_Datatype datatype, int src, int tag, MPI_Comm comm, MPI_Request *request)
int MPI_Irecv_internal(void* buf, int count, MPI_Datatype datatype, int src, int tag, MPI_Comm comm, MPI_Request *request)
{  
   int dst;
...
void CommRecv(Domain& domain, int msgType, Index_t xferFields,
   ...
   if (planeMin && doRecv) {
      ...
      MPI_Irecv_internal(&domain.commDataRecv[pmsg * maxPlaneComm],
              ...
              Index_t dx, Index_t dy, Index_t dz, bool doSend, bool planeOnly)
{
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         ...
         MPI_Isend_internal(destAddr, xferFields*sendCount, baseType,
   ...
   if (!planeOnly) {
      if (rowMin && colMin) {
         ...
         MPI_Isend_internal(destAddr, xferFields*dz, baseType, toRank, msgType,
      ...
      if (rowMin && planeMin) {
         ...
         MPI_Isend_internal(destAddr, xferFields*dx, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg]) ;
         ++emsg ;
      }
      ...
      if (colMin && planeMin) {
         ...
         MPI_Isend_internal(destAddr, xferFields*dy, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg]) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin) {
         ...
         MPI_Isend_internal(comBuf, xferFields, baseType, toRank, msgType,
