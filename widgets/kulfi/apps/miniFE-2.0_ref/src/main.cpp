...
int main(int argc, char** argv) {
...
#if defined(USE_SIGHT)
  SightInit(argc, argv, "miniFE", txt()<<"dbg.miniFE.nx_"<<params.nx<<".ny_"<<params.ny<<".nz_"<<params.nz<<
                                                   ".tol_"<<params.tol<<
                                                   ".mv_overlap_comm_comp_"<<params.mv_overlap_comm_comp<<
                                                   ".use_locking_"<<params.use_locking<<
                                                   ".load_imbalance_"<<params.load_imbalance<<
...
#if defined(KULFI)
  kulfiModularApp modularFibonacci("miniFEApp"); 
#else
  modularApp modularFibonacci("miniFEApp"); 
#endif
#endif
  ...
  {
    module m(instance(txt()<<"Initialization", 0, 6), 
             initOutputs,
             namedMeasures("time", new timeMeasure()));
    m.setOutCtxt(0, module::context("nx", params.nx, "ny", params.ny, "nz", params.nz));
    m.setOutCtxt(1, module::context("tol", params.tol));
    m.setOutCtxt(2, module::context("mv_overlap_comm_comp", params.mv_overlap_comm_comp));
    m.setOutCtxt(3, module::context("use_locking",          params.use_locking));
    m.setOutCtxt(4, module::context("load_imbalance",       params.load_imbalance));
  #if defined(MINIFE_ELL_MATRIX)
    m.setOutCtxt(5, module::context("mtxType",              (char*)"ELL"));
  #else
    m.setOutCtxt(5, module::context("mtxType",              (char*)"CSR"));
  #endif
  }
#endif
