...
#if defined(USE_SIGHT)
, const std::vector<port>& cfgOutPorts
#endif
)
{
...
#if defined(USE_SIGHT)
  std::vector<port> imbalanceOutputs;
#endif
  float largest_imbalance = 0, std_dev = 0;
  {
#if defined(USE_SIGHT)
#if defined(KULFI)
  kulfiModule m(instance(txt()<<"Imbalance", 6, 1), 
           ...
           imbalanceOutputs, module::context("EXP_ID", getenv("EXP_ID")), 
           compNamedMeasures("time", new timeMeasure(), LkComp(2, attrValue::floatT, true)));
#else
  module m(instance(txt()<<"Imbalance", 2, 1), 
           inputs(cfgOutPorts[0],  // nx, ny, nz
                  cfgOutPorts[4]), // load_imbalance
           imbalanceOutputs, namedMeasures("time", new timeMeasure()));
#endif
#endif
...
#if defined(USE_SIGHT)
#if defined(KULFI)
  m.setOutCtxt(0, compContext("imbalance", largest_imbalance, LkComp(2, attrValue::floatT, true),
                              "std_dev",   std_dev,           LkComp(2, attrValue::floatT, true)));
#else
  m.setOutCtxt(0, context("imbalance", largest_imbalance,
                          "std_dev", std_dev));
#endif
#endif
  }
  ...
  size_t global_nnz;
#if defined(USE_SIGHT)
  std::vector<port> dirichiletOutputs;
#endif
  {
#if defined(USE_SIGHT)
#if defined(KULFI)
  kulfiModule m(instance(txt()<<"Dirichilet BC", 7, 1), 
                inputs(cfgOutPorts[0], // nx, ny, nz
                       cfgOutPorts[1], cfgOutPorts[2], cfgOutPorts[3], cfgOutPorts[4], cfgOutPorts[5],
                       imbalanceOutputs[0]), 
                dirichiletOutputs, module::context("EXP_ID", getenv("EXP_ID")), compNamedMeasures("time", new timeMeasure(), LkComp(2, attrValue::floatT, true)));
#else
  module m(instance(txt()<<"Dirichilet BC", 2, 1), 
           inputs(cfgOutPorts[0], // nx, ny, nz
                  imbalanceOutputs[0]), 
           dirichiletOutputs, namedMeasures("time", new timeMeasure()));
#endif
#endif
  ...
  write_vector("b_local.vec", b);
   ...
   global_nnz = compute_matrix_stats(A, myproc, numprocs, ydoc);
#if defined(USE_SIGHT)
#if defined(KULFI)
   m.setOutCtxt(0, compContext("global_nnz", (long)global_nnz, LkComp(2, attrValue::intT, true)));
#else
   m.setOutCtxt(0, context("global_nnz", (long)global_nnz));
#endif
#endif
  }
...
#if defined(USE_SIGHT)
  std::vector<port> cgOutputs;
#endif

  {
#if defined(USE_SIGHT)
#if defined(KULFI)
  kulfiModule m(instance(txt()<<"CG Solve", 9, 3), 
           inputs(port(context("max_iters", max_iters)), 
                  cfgOutPorts[0], cfgOutPorts[1], cfgOutPorts[2], cfgOutPorts[3], cfgOutPorts[4], cfgOutPorts[5], 
                  imbalanceOutputs[0], dirichiletOutputs[0]
                 ), 
           cgOutputs, module::context("EXP_ID", getenv("EXP_ID")), 
           compNamedMeasures("time", new timeMeasure(), LkComp(2, attrValue::floatT, true)));
#else
  module m(instance(txt()<<"CG Solve", 9, 2), 
           inputs(port(context("max_iters", max_iters)),
                  cfgOutPorts[0], cfgOutPorts[1], cfgOutPorts[2], cfgOutPorts[3], cfgOutPorts[4], cfgOutPorts[5], 
                  imbalanceOutputs[0], dirichiletOutputs[0]
                 ),
  ...
  m.setOutCtxt(1, compContext("rnorm", rnorm, noComp()));
#else
  m.setOutCtxt(0, context("rnorm", rnorm);
#endif
  }
