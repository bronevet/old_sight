...
class ModuleThreadInitFinInstantiator: public ThreadInitFinInstantiator
{
  public:
  ...
  ModuleThreadInitFinInstantiator();

  static void initialize();
  static void finalize();
};
ModuleThreadInitFinInstantiator ModuleThreadInitFinInstance;

__thread modularApp* globalModularApps;

ModuleThreadInitFinInstantiator::ModuleThreadInitFinInstantiator() {
  //cout << "ModuleThreadInitFinInstantiator::ModuleThreadInitFinInstantiator()"<<endl;
  addFuncs("Module",
...
void ModuleThreadInitFinInstantiator::initialize() {
  // Create a modularApp for each thread
  if(getenv("DISABLE_MODULARAPP")==NULL)
    globalModularApps = new modularApp("ModularApp");
  else
    globalModularApps = NULL;
}

void ModuleThreadInitFinInstantiator::finalize() {
  //cout << Module_self()<<": ModuleThreadInitFinInstantiator::finalize() *globalComparisons="<<globalComparisons<<endl;
  // Assign each thread to a separate log based on its thread ID
  if(getenv("DISABLE_MODULARAPP")==NULL) {
    assert(globalModularApps != NULL);
    delete globalModularApps;
  }
}
...
void modularApp::setNamedMeasures(const namedMeasures& meas) {
  assert(getInstance());
