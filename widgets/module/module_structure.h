...
// Supports establishment of edges across different logs (which may be generated by different 
// threads or processes). Since each log has its own instance of modularApp with a separate
// ID scheme, we must establish edges among them by 
// - locally keeping track just the local end of the edge
// - during merge finding the other end of the edge and then emitting a complete edge
// We implement this alignment by creating a remoteModuleEdge class that inherits from uniqueMakrer.
// The uniqueMarker stores the information that makes this particular edge unique and both
// the outgoing and incoming sides of the edge must have the same unique ID. This is critical
// for matching the two ends during merge. The remoteModuleEdge class maintains the info that denotes
// the module and port that this side of the edge refers to.
class remoteModuleEdge : public uniqueMark {
  public:
  remoteModuleEdge(const std::string& ID, sight::common::module::ioT type, int index,                       properties* props=NULL);
  remoteModuleEdge(const std::string& ID, sight::common::module::ioT type, int index, const attrOp& onoffOp, properties* props=NULL);

// Sets the properties of this object
  static properties* setProperties(sight::common::module::ioT type, int index, const attrOp* onoffOp, properties* props);
...
class modularApp: public block
{
  ...
  // Sets the named measurements in the currently active modularApp instance
  static void setCompNamedMeasures(const compNamedMeasures& cMeas);
...
class springModularApp// : public compModularApp
{
  ...
  static springModularApp* instance;
...
class ModularAppMerger : public BlockMerger {
  ...
  // When modularApps are used together with comparison and variant tags. The default use-case is for
  // the modularApp to wrap the comparison tag. This means that the modularApp has one outgoing stream,
  // which is shared by its modularAppBody, modularAppStructure and the contents of modularAppStructure.
  // However, since the contents of modularAppBody may be wrapped by comparison and variant tags their
  // outgoing streams may be different from this stream. This is specifically an issue for moduleMarker
  // and remoteModuleEdges tags. Since modules don't respect the separation imposed by comparison tags,
  // we wish to do all module-related processing on the modularApp's outgoing stream and emit the
  // results directly there. As such, we explicitly maintain a copy of the outStreamRecords map
  // along that stream, which maintains pointers to its streamRecords, so that we can use its
  // ModuleStreamRecords for all our accounting.
  protected:
  static std::map<std::string, streamRecord*> moduleOutStreamRecords;

  public:
  static std::map<std::string, streamRecord*>& getModuleOutStreamRecords() { return moduleOutStreamRecords; }
  static ModuleStreamRecord* getMSR() {
    assert(moduleOutStreamRecords.find("module") != moduleOutStreamRecords.end());
    return ((ModuleStreamRecord*)moduleOutStreamRecords["module"]);
  }
...
class RemoteModuleEdgeMerger : public UniqueMarkMerger {
  public:
  RemoteModuleEdgeMerger(std::vector<std::pair<properties::tagType, properties::iterator> > tags,
              std::map<std::string, streamRecord*>& outStreamRecords,
              std::vector<std::map<std::string, streamRecord*> >& inStreamRecords,
              properties* props=NULL);
  ...
  static Merger* create(const std::vector<std::pair<properties::tagType, properties::iterator> >& tags,
                        ...
                        std::vector<std::map<std::string, streamRecord*> >& inStreamRecords,
                        properties* props)
  { return new RemoteModuleEdgeMerger(tags, outStreamRecords, inStreamRecords, props); }
...
class ModuleStreamRecord: public streamRecord {
  ...
  friend class ModularAppStructureMerger;
  friend class ModuleMerger;
  friend class ModuleEdgeMerger;
  friend class RemoteModuleEdgeMerger;
  ...
  class remoteMergePort {
    ...
    remoteMergePort(const std::string& uniqueID, const group& g, sight::common::module::ioT type, int index) :
       uniqueID(uniqueID), g(g), type(type), index(index) {}

    remoteMergePort(const remoteMergePort& that) :
       uniqueID(that.uniqueID), g(that.g), type(that.type), index(that.index) {}

    // We don't use uniqueIDs for comparisons because the only data structure that uses them is
    // remoteEdges, which needs to keep one record for all the edges between two module groups,
    // ignoring their dynamic uniqueIDs.
    bool operator==(const remoteMergePort& that) const
    { return /*uniqueID==that.uniqueID && */g==that.g && type==that.type && index==that.index; }
    ...
    { return /*(uniqueID< that.uniqueID) ||*/
             (/*uniqueID==that.uniqueID && */
                    ...
                    (g==that.g &&
                        (type< that.type ||
                        (type==that.type && index< that.index))))); }

    std::string str() const {
      return string(txt()<<"[remoteMergePort: uniqueID="<<uniqueID<<", g="<<g.str()<<", type="<<(type==sight::common::module::input?"input":"output")<<", index="<<index<<"]");
    }
  }; // class remoteMergePort
  ...
  class mergePort {
    ...
    mergePort(int moduleID, sight::common::module::ioT type, int index) :
       moduleID(moduleID), type(type), index(index) {}

    mergePort(const mergePort& that) :
       moduleID(that.moduleID), type(that.type), index(that.index) {}

    mergePort(int moduleID, const remoteMergePort& that) :
       moduleID(moduleID), type(that.type), index(that.index) {}

    bool operator==(const mergePort& that) const
    { return moduleID==that.moduleID && type==that.type && index==that.index; }

    bool operator<(const mergePort& that) const
    { return (moduleID< that.moduleID) ||
             (moduleID==that.moduleID && type< that.type) ||
             (moduleID==that.moduleID && type==that.type && index<that.index); }

    std::string str() const {
      return string(txt()<<"[mergePort: moduleID="<<moduleID<<", type="<<(type==sight::common::module::input?"input":"output")<<", index="<<index<<"]");
    }
  }; // class mergePort

  // Records all the edges ever observed, mapping them to the pair:
  // - number of times edge's source was observed
  // - the fraction of times we advanced from this source to this destination
  std::map<std::pair<mergePort, mergePort>, std::pair<int, double> > edges;

  const std::map<std::pair<mergePort, mergePort>, std::pair<int, double> >& getEdges() const { return edges; }

  // Records all the remote ports that represent one side of a remote edge but have not been matched
  // to their matching port. Maps the port's uniqueID to the full object.
  std::map<std::pair<std::string, sight::common::module::ioT>, remoteMergePort> remotePorts;

  // Records all the edges formed from remotePorts. remoteEdge tags may appear inside modularAppBody.
  // However, we only align moduleIDs of the modules they connect when we process the modularAppStructure
  // (where module tags are placed). Thus, here we maintain these edges based on their groups, rather
  // than moduleIDs since the IDs may not be known yet.
  std::set<std::pair<remoteMergePort, remoteMergePort> > remoteEdges;

  void addEdge(int fromModuleID, int fromIdx, int toModuleID, int toIdx, int count, double prob);

  void addRemoteEdge(const remoteMergePort& from, const remoteMergePort& to);

  void addRemotePort(const std::string& uniqueID, const group& g, sight::common::module::ioT type, int index);
