...
{ 
  ...
  // node information: HierGraphID:HierGraphName:num_input:num_output:ContainterHierGraphID(parent)
  ...
  for(int i=0; i<lnodes_length; i++)
  {
    ...
    for(int k=0; k<nvi_len; k++)
    {
      ...
      if(mouseX >= stX && mouseX <= enX && mouseY >= stY && mouseY <= enY)
        clickHierGraph(vizInf[0], int(vizInf[1])+k*100, names[i], (100*mouseX/width+10), 100*mouseY/height);
    }
