...
class hierGraph {
  ...
  // A group defines the granularity at which different executions of a hierGraph are differentiated. A group
  //    may be the hierGraph's name, its name+nesting stack within other hierGraphs, name+call stack or name+context.
  //    All hierGraph groups that map to the same group must have the same number of inputs and outputs.
  // Context defines the properties of a given data item propagated from one hierGraph to another. A context is a mapping
  //    from one or more property names to their values. The context of a given group of a hierGraph is the combination
  ...
  //    hierGraph class. This may be a given input (e.g. input0) or the configuration options of a compHierGraph.
