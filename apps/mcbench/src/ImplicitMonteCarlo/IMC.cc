...
// it needs to be a 64 bit int
    ...
    {
        dbg << "size of unsigned long long is less than 64 bit ";
        dbg << "on this machine. exiting..."<< endl;
        exit(0);
    }
	      ...
	      tau_check_message_count++;
                ...
                if( photon_list.empty() )
                {
                    ...
                    if(procID == gather.root_rank())
                    {
                        ...
                        {
                            dbg << "The number of times the code has checked that the IMC" << endl;
                            dbg << "has completed all the particles for this cycle has " << endl;
                            dbg << "exceeded the maximum value of " << maxGlobalCountChecksPerCycle << endl;
                            dbg << "nGlobalCountChecksPerCycle = " << nGlobalCountChecksPerCycle << endl;
                        }
...
//    Need to make sure memory is allocated, etc, before you try to print
    if( !mMaterialDB.isSetUp() )
        dbg << "need to call mMaterialDB.setUp() before IMC::temporalOutput!" << endl;
    if( !mIsSetUp )
        dbg << "need to call IMC::setUp() before IMC::temporalOutput!" << endl;
...
//    otherwise append to currently existing one
    ...
    dbg.precision(6);
    dbg << "cycle = " << cycleNumber << "  ";
    dbg << "tn = " << tn << "  dt = " << dt << endl;
    dbgprintf("  Npart = %llu\n", N_advanced );
...
//    Need to make sure memory is allocated, etc, before you try to print
    if( !mMaterialDB.isSetUp() )
        dbg << "need to call mMaterialDB.setUp() before IMC::spatialOutput!" << endl;
    if( !mIsSetUp )
        dbg << "need to call IMC::setUp() before IMC::spatialOutput!" << endl;
...
//    Need to make sure memory is allocated, etc, before you try to print
    if( !mMaterialDB.isSetUp() )
        dbg << "need to call mMaterialDB.setUp() before IMC::computeMaxDeviationFromAnalytic!" << endl;
    if( !mIsSetUp )
        dbg << "need to call IMC::setUp() before IMC::computeMaxDeviationFromAnalytic!" << endl;
...
//    number of scatterings per zone
    ...
    if( root == procID )
    {
       ...
       dbg << "mean free path = " << shave( d_total/static_cast<double>(gN_collisions+1) );
       dbg << '\n' << '\n';
    }
    ...
    {
        dbg << "WARNING - DIAGNOSTIC PROBLEM!" << '\n';
        dbg << "Problem with photon number accounting!" << '\n';
        dbg << "Number in census = census_photons.size()" << '\n';
        dbg << "does not equal the number in - number lost!" << '\n';
        ...
        dbg << "Energy actually in census photons = " << E_actually_in_census << '\n';
    } 
    if( root == procID )
    {
      dbg << '\n';
    ...
    if( root == procID )
    {
      dbg << "IMC Energy accounting procID="<<procID<<":\n\n";
    }
...
#pragma omp master
    {
        ...
        double scat = static_cast<double>(gN_collisions)/static_cast<double>(gN_zone_crossings + 1);
        {
            // Emit this text at medium or high debug level
            attrIf aif(new attrGE("verbose", (long)1));

            dbgprintf( "\nscatters per zone crossing = %f\n", scat); 
            dbgprintf( "mean free path = %f\n" , shave( d_total_sum/static_cast<double>(gN_collisions+1) ) ); 
            dbgprintf("\n\n");
       }
       traceAttr("Trace", "scat_per_z_cross", attrValue((double)scat));
       traceAttr("Trace", "mean_fr_path", attrValue((double)shave( d_total_sum/static_cast<double>(gN_collisions+1) )));
    }//end of omp master section
    ...
    {
        dbgprintf( "mpi: %d, tid: %d, WARNING - DIAGNOSTIC PROBLEM! Problem with photon number accounting!\n", procID, tid);
        dbgprintf( "mpi: %d, tid: %d, Number in census = census_photons.size() does not equal the number in - number lost!\n", procID, tid);
        dbgprintf( "mpi: %d, tid: %d, N_remaining_threadSum=%llu, N_in_census_threadSum=%llu\n",
        ...
        dbgprintf( "tid: %d, Energy actually in census photons = %f\n", tid, E_actually_in_census);
    } 
    dbgprintf("\n");

    //    Info on IMC and total energy balance
    // Only emit this text at high debug level
    scope imcS("IMC Energy accounting:\n\n", attrGE("verbose", (long)2));
    ...
    { 
      ...
      dbgprintf("\n\n");
      dbgprintf("Source Energy accounting:\n");
    }
