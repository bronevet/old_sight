...
{
    dbg << endl;
    dbg << "Source_data_base diagnostic information:" << endl;
    dbg << endl;

    dbg << "cummulative photon number information:" << endl;
    dbgprintf("N_initial = %llu\n", _N_initial );
    dbgprintf("N_created = %llu\n", _N_created );
    dbgprintf("N_source_RR_destroyed = %llu\n", N_source_RR_destroyed );
    unsigned long long N_passed_to_IMC;
    N_passed_to_IMC = _N_initial + _N_created - N_source_RR_destroyed;
    dbgprintf("N passed to IMC =  %llu\n", N_passed_to_IMC );
    dbg << endl;

    dbg << "source diagnostic info:" << endl;
    ...
    dbg << "cummulative photon energy information:" << endl;
    dbg << "E_in_photons = ";
    dbg << E_in_photons << endl;
    dbg << "E_photons_total = ";
    dbg << E_photons_total << endl;
    dbg << "E_emitted_total = " << E_emitted_total << endl;
    dbg << "E_source_RR_created = " << E_source_RR_created << endl;
    dbg << "E_source_RR_destroyed = " << E_source_RR_destroyed << endl;
    double delta_E_RR = E_source_RR_created - E_source_RR_destroyed;
    dbg << "delta_E_RR = " << delta_E_RR << endl;
    double E_difference = E_photons_total - delta_E_RR - E_in_photons;
    dbg << "E_difference = " << E_difference << endl;
    dbg << endl;

    dbg << "Source energy balance = " << energyBalance() << endl;
    dbg << endl;
}
...
#pragma omp barrier
    ...
    if( tid == 0 && my_id == 0)
    {
        // Only emit this text at high debug level 
        attrIf aif(new attrGE("verbose", (long)2));

        dbgprintf("\n");
        dbgprintf("Source_data_base diagnostic information:\n");
        dbgprintf("\n");

        dbgprintf("cumulative photon number information:\n");
    }
...
#pragma omp barrier 
...

    {
        ...
        dbgprintf("N passed to IMC =  %llu\n", N_passed_to_IMC );
        dbgprintf("\n");
        dbgprintf("source diagnostic info:\n");
    }
    {
        traceAttr("Photon Number", "N_initial", attrValue((double)ull_diags[0]));
        traceAttr("Photon Number", "N_created", attrValue((double)ull_diags[1]));
        traceAttr("Photon Number", "N_source_RR_destroyed", attrValue((double)ull_diags[2]));
        traceAttr("Photon Number", "N passed to IMC", attrValue((double)N_passed_to_IMC));
    }
...
#pragma omp barrier 
    ...
    
    {
        ...
        dbgprintf("cumulative photon energy information:\n");
        dbgprintf("E_in_photons = %f\n",sum_E_in_photons);
        dbgprintf("E_photons_total = %f\n",sum_E_photons_total);
        dbgprintf("E_emitted_total = %f\n",sum_E_emitted_total);
        dbgprintf("E_source_RR_created = %f\n",sum_E_source_RR_created);
        dbgprintf("E_source_RR_destroyed = %f\n",sum_E_source_RR_destroyed);
        dbgprintf("delta_E_RR = %f\n",delta_E_RR);
        dbgprintf("E_difference = %g\n",E_difference);
        dbgprintf("\n");
        dbgprintf("Source energy balance = %g\n",E_balance);
        dbgprintf("\n");
    }
    {
        traceAttr("Photon Energy", "E_in_photons", attrValue((double)sum_E_in_photons));
        traceAttr("Photon Energy", "E_photons_total", attrValue((double)sum_E_photons_total));
        traceAttr("Photon Energy", "E_emitted_total", attrValue((double)sum_E_emitted_total));
        traceAttr("Photon Energy", "E_source_RR_created", attrValue((double)sum_E_source_RR_created));
        traceAttr("Photon Energy", "E_source_RR_destroyed", attrValue((double)sum_E_source_RR_destroyed));
        traceAttr("Photon Energy", "delta_E_RR", attrValue((double)delta_E_RR));
        traceAttr("Photon Energy", "E_difference", attrValue((double)E_difference));
        traceAttr("Photon Energy", "Source energy balance", attrValue((double)E_balance));
    }
