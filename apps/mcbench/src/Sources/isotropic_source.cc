...
//    Go over every element of mesh_zones
    ...
    {
        dbg << "sourceStrength = " << sourceStrength;
        dbg << " in isotropic source ctor! "<< endl;
        dbg << "It must be >= 0.0 !" << endl;
    }
...
diagnostic_output() const
{
    ...
    dbg << "for source " << source_name << endl;
    dbg << "N_photons_created = " << N_photons_created << endl;
    dbg << "E_in_physical = " << E_in_physical << endl;
    dbg << "E_in_photons = " << E_in_photons << endl;
    ...
    dbg << "fractional Energy input error = " << E_error << endl;
    dbg << endl;
    }

    traceAttr("Source Diag", txt()<<source_name<<": N_photons_created", attrValue((double)N_photons_created));
    traceAttr("Source Diag", txt()<<source_name<<": E_in_physical", attrValue((double)E_in_physical));
    traceAttr("Source Diag", txt()<<source_name<<": E_in_photons", attrValue((double)E_in_photons));
    traceAttr("Source Diag", txt()<<source_name<<": E_error", attrValue((double)E_error));
}
...
#pragma omp barrier 
...

    {
      // Only emit this text at high debug level
      attrIf aif(new attrGE("verbose", (long)2));
        dbgprintf("for source %s\n",source_name.c_str());
        dbgprintf("N_photons_created = %llu\n",sum_N_photons_created);
        dbgprintf("E_in_physical = %f\n",sum_E_in_physical);
    ...
    if( tid == 0 && my_id == 0)
    {
        dbgprintf("fractional Energy input error = %f\n",E_error);
        dbgprintf("\n");
    }
    traceAttr("Source Diag", txt()<<source_name<<": N_photons_created", attrValue((double)sum_N_photons_created));
    traceAttr("Source Diag", txt()<<source_name<<": E_in_physical", attrValue((double)sum_E_in_physical));
    traceAttr("Source Diag", txt()<<source_name<<": E_in_photons", attrValue((double)sum_E_in_photons));
    traceAttr("Source Diag", txt()<<source_name<<": E_error", attrValue((double)E_error));
