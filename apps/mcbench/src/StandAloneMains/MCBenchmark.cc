...
int main(int argc, char* argv[])
{
  ...
  if (mpiTaskID == 0) 
    dbgprintf ("MPI_threaded provided  = %d \n", provided );
#else
  ...
  attr rankAttr("rank", (long)mpiTaskID);
  attr tidAttr("threadID", (long)0);
  
  // We only emit debug output on rank 0 / thread 0
  attrIf aif(new attrEQ("rank", (long)0));
  attrAnd aAnd(new attrEQ("threadID", (long)0));
  ...
  if (mpiTaskID == 0) 
    dbg<<"max num omp threads= "<<num_threads<<endl;
#endif

  if (mpiTaskID == 0) {
    dbgprintf ("\nMCB version 1.0.3, July 22, 2013.\n\n" );
    dbg << "nMpiTasks = " << nMpiTasks << endl;
  }
  //---------------------------------------------------------------------//
  ...
  if( mpiTaskID == 0)
    dbg<<"isWeakScaling="<<(isWeakScaling==1?"True":"False")<<endl;
  if(num_threads > 0) omp_set_num_threads(num_threads);
  if (mpiTaskID == 0) {
    int omp_maxthreads;
    omp_maxthreads= omp_get_max_threads();
    dbg<<"number of threads used = "<<num_threads<<endl;
  }

#define OUTPUT_ARGS
#ifdef OUTPUT_ARGS
  if( mpiTaskID == 0) {
    scope s("After call to parseArgs");
    dbg<<"    nMpiTasksX=       "<<nMpiTasksX<<endl;
    dbg<<"    nMpiTasksY=       "<<nMpiTasksY<<endl;
    dbg<<"    nZonesXGlobal= "<<nZonesXGlobal<<endl;
    dbg<<"    nZonesYGlobal= "<<nZonesYGlobal<<endl;
    dbg<<"    xMaxGlobal=    "<<xMaxGlobal<<endl;
    dbg<<"    yMaxGlobal=    "<<yMaxGlobal<<endl;
    dbg<<"    sigmaAbsorb=   "<<sigmaAbsorb<<endl;
    dbg<<"    sigmaScatt=    "<<sigmaScatt<<endl;
    dbg<<"    xRoot     =    "<<xRoot<<endl;
    dbg<<"    yRoot     =    "<<yRoot<<endl;
    dbg<<"    numParticles=  "<<N_particles<<endl;
    dbg<<"    xInnerLo     = "<<xInnerLo<<endl;
    dbg<<"    xInnerHi     = "<<xInnerHi<<endl;
    dbg<<"    yInnerLo     = "<<yInnerLo<<endl;
    dbg<<"    yInnerHi     = "<<yInnerHi<<endl;
    dbg<<"    ratioSigmaInner = "<<ratioSigmaInner<<endl;
    dbg<<"    isDistribSource= "<<isDistribSource<<endl;
    dbg<<"    isWeakScaling= "<<isWeakScaling<<endl;
    dbg<<"    isMirrorBoundary= "<<isMirrorBoundary<<endl;
    dbg<<"    isMultiOpacity= "<<isMultiOpacity<<endl;
  }
#endif
  ...
  if( nMpiTasksX * nMpiTasksY != nMpiTasks)
  {
    if( mpiTaskID == 0 )
    {
      dbg << "ERROR: Must have nMpiTasksX * nMpiTasksY = nMpiTasks." << '\n';
      dbg << "nMpiTasksX = " << nMpiTasksX << endl;
      dbg << "nMpiTasksY = " << nMpiTasksY << endl;
      dbg << "nMpiTasksX*nMpiTasksY = " << nMpiTasksX*nMpiTasksY << endl;
      dbg << "nMpiTasks = " << nMpiTasks << endl;
...
#pragma omp parallel default(shared) private(tid)
  ...
  contextAttrs.push_back("time");
  trace statsTrace("Trace", contextAttrs, trace::showEnd, trace::table);
  trace statsIMCE("IMC Energy", contextAttrs, trace::showEnd, trace::table);
  trace statsPhotN("Photon Number", contextAttrs, trace::showEnd, trace::table);
  trace statsSrcDiag("Source Diag", contextAttrs, trace::showEnd, trace::table);
  trace statsPhotE("Photon Energy", contextAttrs, trace::showEnd, trace::table);

  {
  // Emit a small amount of debug output during the main iteration
  attr verboseAttr("verbose", (long)1);
  do
  {
    attr timeAttr("time", (double)t);
    const unsigned int interval = 1;
    attr imAttr("iteration_mod_interval", (long)(iteration % interval));
    attrAnd aPeriod(new attrEQ("iteration_mod_interval", (long)0));
    scope tsScp(txt() << "cycle = " << iteration << " t = " << t);
...
#pragma omp parallel default(shared) private(tid)
  ...
  // Emit detailed debug output after the main iteration
  attr verboseAttr("verbose", (long)2);
...
#pragma omp critical (globalTracks)
  ...
  {
    dbg << '\n';
    dbg << "MC advance time ="<<advanceTime<<endl;
    dbg <<" MC max error ="<<maxError<<endl;
    dbg <<" globalTracks="<<globalTracksSum<<endl;
    dbg <<" FOM ="<<globalTracksSum/advanceTime<<" tracks/second"<<endl;
    dbg <<" performance per core ="<<globalTracksSum/advanceTime/(nCores*nMpiTasks)<<" tracks/second/core"<<endl;
    dbg << "done!" << endl;
  }
...
//     opt->noPOSIX);  // Do not check for POSIX style character options
  ...
  if( myRank==0 ) {
    dbg<<"nCoresX = "<<nCoresX<<endl;
    dbg<<"nCoresY = "<<nCoresY<<endl;
  }
  ...
  if( opt->getFlag("distributedSource") ) {
    ...
    if( myRank==0 ) dbg<<"Using a distributed source"<<endl;
  } else {
    isDistribSource = false;
    if( myRank==0 ) dbg<<"Using a point source"<<endl;
  }
  ...
  if( isWeakScaling )
  {
    ...
    if( myRank==0 )
      dbg<<"N_particles = "<<N_particles<<endl;
