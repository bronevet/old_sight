...
// Function prototypes
static void 
DumpDomainToVisit(DBfile *db, Domain& domain, int myRank, sightModule* m, int outputNum, std::vector<int>& nodeDiscretizationBasis, std::vector<int>& edgeDiscretizationOrigin, std::vector<int>& edgeDiscretizationBasis);
static
...
void DumpToVisit(Domain& domain, int numFiles, int myRank, int numRanks, std::vector<int>& nodeDiscretizationBasis, std::vector<int>& edgeDiscretizationOrigin, std::vector<int>& edgeDiscretizationBasis) 
{
  ...
  DumpDomainToVisit(db, domain, myRank, NULL, 0);
  ...
  if (db) {
     ...
     DumpDomainToVisit(db, domain, myRank, NULL, 0, nodeDiscretizationBasis, edgeDiscretizationOrigin, edgeDiscretizationBasis);
...
void DumpDomainToModule(Domain& domain, int myRank, sightModule* m, int outputNum, std::vector<int>& nodeDiscretizationBasis, std::vector<int>& edgeDiscretizationOrigin, std::vector<int>& edgeDiscretizationBasis) {
   DumpDomainToVisit(NULL, domain, myRank, m, outputNum, nodeDiscretizationBasis, edgeDiscretizationOrigin, edgeDiscretizationBasis);
}

void DumpDomainToVisit(DBfile *db, Domain& domain, int myRank, sightModule* m, int outputNum, std::vector<int>& nodeDiscretizationBasis, std::vector<int>& edgeDiscretizationOrigin, std::vector<int>& edgeDiscretizationBasis)
{
   int ok = 0;
   compContext outCtxt;
   ...
   for (int ni=0; ni < domain.numNode() ; ++ni) {
      coords[0][ni] = double(domain.x(ni)) ;
      coords[1][ni] = double(domain.y(ni)) ;
      coords[2][ni] = double(domain.z(ni)) ;
   }
   if(db) {
   ...
   double *e = new double[domain.numElem()] ; 
   for (int ei=0; ei < domain.numElem(); ++ei) {
      e[ei] = double(domain.e(ei)) ;
   }
   if(db)
   ...
   double *yd    = new double[domain.numNode()];
   double *xd    = new double[domain.numNode()];
   double *speed = new double[domain.numNode()];
   
   for(int ni=0 ; ni < domain.numNode() ; ++ni) {
      xd[ni]    = double(domain.xd(ni));
      yd[ni]    = double(domain.yd(ni));
      zd[ni]    = double(domain.zd(ni));
      speed[ni] = double(sqrt((xd[ni]*xd[ni])+(yd[ni]*yd[ni])+(zd[ni]*zd[ni])));
   }
