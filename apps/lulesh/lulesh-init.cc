...
Domain::Domain(Int_t numRanks, Index_t colLoc,
               ...
               Real_t dtfixed_arg, Real_t dthydro_arg, Real_t dtcourant_arg, Real_t dtmax_arg)
   :
   ...
   m_refdens(Real_t(1.0))
{
   ...
   m_numEdgeElems = nx ;
   m_numEdgeNodes = m_numEdgeElems+1 ;
   this->cost() = cost;
   ...
   m_sizeX = m_numEdgeElems ;
   m_sizeY = m_numEdgeElems ;
   m_sizeZ = m_numEdgeElems ;
   m_numElem = m_numEdgeElems*m_numEdgeElems*m_numEdgeElems ;

   m_numNode = m_numEdgeNodes*m_numEdgeNodes*m_numEdgeNodes ;
   ...
   SetupCommBuffers(m_numEdgeNodes);
   ...
   BuildMesh(nx, m_numEdgeNodes, m_numEdgeElems);
   ...
   SetupSymmetryPlanes(m_numEdgeNodes);

   // Setup element connectivities
   SetupElementConnectivities(m_numEdgeElems);

   // Setup symmetry planes and free surface boundary arrays
   SetupBoundaryConditions(m_numEdgeElems);
   ...
   dtfixed() = dtfixed_arg;//Real_t(-1.0e-6) ; // Negative means use courant condition
   stoptime()  = Real_t(1.0e-2); // *Real_t(m_numEdgeElems*tp/45.0) ;

   // Initial conditions
   deltatimemultlb() = Real_t(1.1) ;
   deltatimemultub() = Real_t(1.2) ;
   dtcourant() = /*dtcourant_arg; //*/ Real_t(1.0e+20) ;
   dthydro()   = /*dthydro_arg, //*/ Real_t(1.0e+20) ;
   dtmax()     = dtmax_arg; //Real_t(1.0e-2) ;
   time()    = Real_t(0.) ;
   cycle()   = Int_t(0) ;
