...
void CG( const Operator &A, const Vector &b, Vector &x,
   ...
   if (den <= 0.0) {
      if (nom0 > 0.0)
         dbg <<"Operator A is not postive definite. (Ar,r) = "
              << den << endl;
      return;
   }
   ...
   for(i = 1; i<max_num_iter ;i++) {
      ...
      if (print_iter == 1)
         dbg << "   Iteration : " << setw(3) << i << "  (r, r) = "
              << betanom << endl;

      if ( betanom < r0) {
         if (print_iter == 2)
            dbg << "Number of CG iterations: " << i << endl;
         else
            if (print_iter == 3)
               dbg << "(r_0, r_0) = " << nom0 << endl
      ...
      {
         if (d * d > 0.0)
            dbg <<"Operator A is not postive definite. (Ad,d) = "
                 << den << endl;
      }
