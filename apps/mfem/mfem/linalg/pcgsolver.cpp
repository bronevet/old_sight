           ...
           int save = 0)
{
   ...
   if (den < 0.0)
   {
      dbg << "Negative denominator in step 0 of PCG: ";
      dbg << den << endl;
      //    return;
   }
   ...
   {
      attr iterAttr("iter", i);
      
      if (save)
         if (i % save == 0)
         {
            dbg << "saving the solution vector on iteration " << i << endl;
            ofstream out("pcg.x");
            x.Print(out, 1);
         }
      ...
      {
         if (print_iter >= 2)
            dbg << "Number of PCG iterations: " << i << endl;
         else
            if (print_iter >= 3)
               dbg << "(B r_0, r_0) = " << nom0 << endl
          ...
          double RTOLERANCE=10e-12, double ATOLERANCE=10e-24)
{
   ...
   for(i = 1; i < max_num_iter ; i++) {
      ...
      if (nom < r0) {
         if (print_iter == 2)
            dbg << "Number of iterations: " << i << endl
                 << "Conv. rate: " << cf << endl;
         else
            if (print_iter == 3)
               dbg << "(B r_0, r_0) = " << nom0 << endl
