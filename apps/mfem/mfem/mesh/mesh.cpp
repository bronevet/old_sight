...
void Mesh::PrintCharacteristics(Vector *Vh, Vector *Vk)
{
   ...
   dbg << "Mesh Characteristics:" << flush;
...
#ifdef MFEM_DEBUG
         dbg << "Mesh::Load : redges = " << redges << endl;
#endif
...
void Mesh::CheckElementOrientation()
{
   ...
   if (wo > 0)
      dbg << "Orientation fixed in " << wo << " of "<< NumOfElements
           << " elements" << endl;
#endif
}
...
#ifdef MFEM_DEBUG
   if (wo > 0)
      dbg << "Orientation fixed in " << wo << " of "<< NumOfBdrElements
...
#ifdef MFEM_DEBUG
      dbg << "Mesh::GeneratePartitioning(...): edgecut = "
           << edgecut << endl;
#endif
   }
...
void Mesh::LocalRefinement(const Array<int> &marked_el, int type)
{
   ...
   else if (Dim == 3) // ---------------------------------------------------
   {
      ...
      // dbg << "Maximum generation: " << max_gen << endl;
