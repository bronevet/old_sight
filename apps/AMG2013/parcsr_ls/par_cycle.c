         ...
         )
{
 ...
 /* Local variables  */ 
   ...
   while (Not_Finished)
   {
      ...
      sightModule modCycle(instance("V Iter ", 2, 0), 
                     inputs(port(runCfg), 
                            port(context("local_size",  local_size,
                                         "level",       level,
                                         "nnz",         (long)hypre_ParCSRMatrixNumNonzeros(A_array[level]),
                                         "numVars",     hypre_ParCSRMatrixDNumNonzeros(A_array[level]),
      ...
      // If this is the first iteration in this V-cycle
      if(iter == 0) {
        // If this is not the first V-cycle
        /*if(lastAnchor != anchor::noAnchor)
          AMGVCycleGraph.addUndirEdge(lastAnchor, sCycle.getAnchor());*/
      // If this is not the first iteration in this V-cycle
      } else {
        //dbg << iter << ": cycle_param="<<cycle_param<<endl;
        // If coarsening, add edge from last anchor to this one
        if(cycle_param==1) AMGVCycleGraph.addDirEdge(lastAnchor, sCycle.getAnchor());
        // If refining, add an edge from this anchor to the last one
        else if(cycle_param==2) AMGVCycleGraph.addDirEdge(sCycle.getAnchor(), lastAnchor);
        // If solving, add edge from last one to this one
        else if(cycle_param==3)  AMGVCycleGraph.addDirEdge(lastAnchor, sCycle.getAnchor());
      }
      lastAnchor = sCycle.getAnchor();
