...
hypre_BoomerAMGCreate()
{
   ...
   hypre_ParAMGDataMaxCoarseSize(amg_data) = coarse_threshold;
   hypre_ParAMGDataSeqData(amg_data) = NULL;
   ...
   hypre_ParAMGDataSmootherPrec(amg_data) = NULL;
   ...
   hypre_ParAMGDataThetaEst(amg_data) = NULL;
...
hypre_BoomerAMGDestroy( void *data )
{
   ...
   HYPRE_Solver *smoother_prec = hypre_ParAMGDataSmootherPrec(amg_data);

   //seq solver?
 if  (hypre_ParAMGDataSeqData(amg_data))
   {
      hypre_SeqAMGData *seq_data = hypre_ParAMGDataSeqData(amg_data);
      int tot_levels = hypre_SeqAMGDataNumLevels(seq_data);
      if (hypre_SeqAMGDataParticipate(seq_data))
      {
         for (i=num_levels-1; i < tot_levels; i++)
         {
            hypre_SeqVectorDestroy(hypre_SeqAMGDataFArray(seq_data)[i]);
            hypre_SeqVectorDestroy(hypre_SeqAMGDataUArray(seq_data)[i]);
            hypre_CSRMatrixDestroy(hypre_SeqAMGDataAArray(seq_data)[i]);
         }
         for (i=num_levels-1; i < tot_levels-1; i++)
         {
            hypre_CSRMatrixDestroy(hypre_SeqAMGDataPArray(seq_data)[i]);
         }
         for (i=num_levels-1; i < tot_levels-1; i++)
         {
            hypre_TFree(hypre_ParAMGDataCFMarkerArray(amg_data)[i]);
         }
         for (i=num_levels; i < tot_levels; i++)
            hypre_TFree(hypre_ParAMGDataDofFuncArray(amg_data)[i]);
         
         
         hypre_TFree(hypre_SeqAMGDataFArray(seq_data));
         hypre_TFree(hypre_SeqAMGDataUArray(seq_data));
         hypre_TFree(hypre_SeqAMGDataAArray(seq_data));
         hypre_TFree(hypre_SeqAMGDataPArray(seq_data));
         
         hypre_SeqVectorDestroy(hypre_SeqAMGDataVtemp(seq_data));
      }
      

      hypre_TFree(seq_data);
   ...
   if (hypre_ParAMGDataGridRelaxType(amg_data))
   {
      ...
      else if (grid_relax_type[1] == 14 || grid_relax_type[3] == 14 )
      {
         ...
         hypre_TFree (hypre_ParAMGDataSmoother(amg_data));
         hypre_ParAMGDataSmoother(amg_data) = NULL;
         hypre_TFree (hypre_ParAMGDataSmootherPrec(amg_data));
         hypre_ParAMGDataSmootherPrec(amg_data) = NULL;
   ...
   if (hypre_ParAMGDataThetaEst(amg_data))
   {
      hypre_TFree(hypre_ParAMGDataThetaEst(amg_data));
      hypre_ParAMGDataThetaEst(amg_data) = NULL;
   }
   ...
   {
      if (hypre_ParAMGDataSmoothType(amg_data) == 6)
      {
...
int
hypre_BoomerAMGSetPMax1( void     *data,
                            int    P_max1 )
{
   hypre_ParAMGData  *amg_data = (hypre_ParAMGData*)data;
   ...
   hypre_ParAMGDataPMax1(amg_data) = P_max1;

   return hypre_error_flag;
}

int
hypre_BoomerAMGSetPMax2( void     *data,
                            int    P_max2 )
{
   hypre_ParAMGData  *amg_data = (hypre_ParAMGData*)data;
   ...
   hypre_ParAMGDataPMax2(amg_data) = P_max2;
...
int
hypre_BoomerAMGSetChebyEigRatio( void     *data,
                              double      ratio)
{
   hypre_ParAMGData  *amg_data = (hypre_ParAMGData*)data;
   ...
   hypre_ParAMGDataChebyEigRatio(amg_data) = ratio;
                                   ...
                                   int thresh)
{
   ...
   hypre_ParAMGDataMaxCoarseSize(amg_data) = thresh;
                                   ...
                                   int thresh)
{
   ...
   hypre_ParAMGDataSeqThreshold(amg_data) = thresh;
