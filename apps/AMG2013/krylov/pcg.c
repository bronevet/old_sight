...
/* default preconditioner must be set here but can be changed later... */
   pcg_functions->precond_setup = (HYPRE_PtrToSolverFcn)PrecondSetup;
   pcg_functions->precond       = (HYPRE_PtrToSolverFcn)Precond;
                ...
                context& runCfg, int& num_precond_calls)
{
   ...
   HYPRE_PtrToSolverFcn precond_setup = (HYPRE_PtrToSolverFcn)(pcg_functions -> precond_setup);
   HYPRE_PtrToSolverFcn precond_data = (HYPRE_PtrToSolverFcn)(pcg_data -> precond_data);
   int            ierr = 0;
...
*/
   ...
   HYPRE_PtrToSolverFcn precond   = (pcg_functions -> precond);
...
//   int num_precond_calls = 0;
   ...
   else
   {
      ...
      precond((HYPRE_Solver)precond_data, (HYPRE_Matrix)A, (HYPRE_Vector)b, (HYPRE_Vector)p, runCfg, AMGVCycleGraph, lastAnchor, solverCtxt, num_precond_calls);
      num_precond_calls++;
...
//      solveModule->setOutCtxt(0, context("num_precond_calls", num_precond_calls));
   ...
   (*(pcg_functions->ClearVector))(p);
   context solverCtxt("i", -1);
   precond((HYPRE_Solver)precond_data, (HYPRE_Matrix)A, (HYPRE_Vector)r, (HYPRE_Vector)p, runCfg, AMGVCycleGraph, lastAnchor, solverCtxt, num_precond_calls);
   num_precond_calls++;
...
//      solveModule->setOutCtxt(0, context("num_precond_calls", num_precond_calls));
   ...
   if ( print_level > 1 && my_id==0 )  /* formerly for par_csr only */
   {
      ...
      {
         if ( stop_crit && !rel_change && atolf==0 ) {  /* pure absolute tolerance */
            dbgprintf("Iters       ||r||_2     conv.rate\n");
            dbgprintf("-----    ------------   ---------\n");
         }
         else {
            dbgprintf("Iters       ||r||_2     conv.rate  ||r||_2/||b||_2\n");
            dbgprintf("-----    ------------   ---------  ------------ \n");
         }
      }
      else  /* !two_norm */
      {
         dbgprintf("Iters       ||r||_C      ||r||_C/||b||_C\n");
         dbgprintf("-----    ------------    ------------ \n");
      }
   }
   }
   ...
   while ((i+1) <= max_iter)
   {
     ...
     context solverCtxt("i", i);
      ...
      precond((HYPRE_Solver)precond_data, (HYPRE_Matrix)A, (HYPRE_Vector)r, (HYPRE_Vector)s, runCfg, AMGVCycleGraph, lastAnchor, solverCtxt, num_precond_calls);
      num_precond_calls++;
