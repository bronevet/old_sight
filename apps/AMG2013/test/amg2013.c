                ...
                ProblemData  *data_ptr )
{
   ...
   {
      printf("Error: Invalid number of processes or process topology! pool_procs[%d] = %d, num_procs = %d\n", data.npools, pool_procs[data.npools], num_procs);
      exit(1);
   }
      ...
      char *argv[] )
{
   ...
   while ( (arg_index < argc))
   {
      ...
      else if ( strcmp(argv[arg_index], "-solver") == 0 )
      {
         arg_index++;
         char* endptr;
         solver_id = strtol(argv[arg_index], &endptr, 10);
         // If the solver_id was specified as a number 
         if(argv[arg_index] != endptr) {
            ...
            switch(solver_id) {
              case 0: solverName = "PCG_AMG"; break;
              case 1: solverName = "PCG_Diag"; break;
              case 2: solverName = "GMRES_AMG"; break;
              case 3: solverName = "GMRES_Diag"; break;
            }
         } else {
                 if(strcmp(argv[arg_index], "PCG_AMG") == 0)  solver_id = 0;
            else if(strcmp(argv[arg_index], "PCG_Diag") == 0) solver_id = 1;
            else if(strcmp(argv[arg_index], "GMRES_AMG") == 0)  solver_id = 2;
            else if(strcmp(argv[arg_index], "GMREs_Diag") == 0) solver_id = 3;
            solverName = argv[arg_index];
         }
         arg_index++;
      }
      else if ( strcmp(argv[arg_index], "-P") == 0 )
      {
         arg_index++;
         for (j = 0; j < 3; j++)
            P[j] = atoi(argv[arg_index++]);
      }
      else if ( strcmp(argv[arg_index], "-r") == 0 )
      {
         arg_index++;
         for (j = 0; j < 3; j++)
            r[j] = atoi(argv[arg_index++]);
      }
                   ...
                   HYPRE_ParVector  *x_ptr)
{
   ...
   if ((P*Q*R) != num_procs)
   {
      printf("Error: Invalid number of processors or processor topology. P=%d, Q=%d, R=%d, num_procs=%d\n", P, Q, R, num_procs);
      exit(1);
   }
