...
for (i = 0; i < ROUNDS; i++) {
	{ scope s(txt()<< "Round " << i);
   /* All tasks calculate pi using dart board algorithm */
   homepi = dboard(DARTS);
   { scope s(txt()<<"Calculating pi using dart board algorithm", scope::minimum);
   }
   ...
   if (taskid != MASTER) {
      ...
      printf("   After %8d throws, average value of pi = %10.8f\n",
               (DARTS * (i + 1)),avepi);
      { scope s(txt()<<"Calculating average value of pi", scope::minimum);
      	  dbg << "After " << DARTS * (i + 1) << " throws, average value of pi = " << avepi;
      }
      }
	}
	if(i<ROUNDS-1) { MPI_Barrier(MPI_COMM_WORLD); }
}

if (taskid == MASTER){
   printf ("\nReal value of PI: 3.1415926535897 \n");
   { scope s(txt()<<"Real value of PI: 3.1415926535897", scope::minimum);
   }
}
MPI_Finalize();
return 0;
