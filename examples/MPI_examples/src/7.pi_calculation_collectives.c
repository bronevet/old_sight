...
for (i = 0; i < ROUNDS; i++) {
   { scope s(txt()<<"Round " << i);
	   ...
	   homepi = dboard(DARTS);
	   { scope s(txt() << "Calculate pi using dartboard algorithm", scope::minimum);
	   }
	   ...
	   if (taskid == MASTER) {
		  ...
		  printf("   After %8d throws, average value of pi = %10.8f\n",
				  (DARTS * (i + 1)),avepi);
		  { scope s(txt() << "Computing average");
			  dbg << "After " <<  (DARTS * (i + 1)) << " throws, average value of pi = " << avepi << endl;
		  }
	   }
  }
