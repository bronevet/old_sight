...
{ scope s(txt()<< "MPI Point-to-point communication timings for any even number of MPI tasks");
	dbg << "Message start size= " << STARTSIZE << " bytes" << endl;
	dbg << "Message finish size= " << ENDSIZE << " bytes" << endl;
	dbg << "Incremented by " << INCREMENT << " bytes per iteration" << endl;
	dbg << "Roundtrips per iteration= " << ROUNDTRIPS << endl;
}
...
{ scope s(txt() << "Send host name to task 0");
...
{ scope s(txt() << "Determine my send/receive partner", scope::minimum);
	dbg << "Task " << dest < " is my partner";
}
{ scope s(txt() << "Tell task 0 who is my partner");
...
if (rank < numtasks/2) {
  for (n=start; n<=end; n=n+incr) {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    { scope s(txt() << "Message size in this iteration " << n);
		for (i=1; i<=rndtrps; i++){
			{ scope s(txt() << "#" << i << " roundtrip");
			  t1 = MPI_Wtime();
    ...
    if (rank == 0) {
      ...
      /* only two tasks. */
      { scope s(txt()<< "Receive timing from other tasks");
		  for (j=1; j<numtasks/2; j++)
			MPI_Recv(&timings[j], 3, MPI_DOUBLE, j, tag, MPI_COMM_WORLD, &status);
      }
      scope(txt()<<"Printing timing results..", scope::minimum);
      ...
      scope(txt() << "Overall averages: best: "<< bestall/(numtasks/2) << " / avg: " << avgall/(numtasks/2) << " / worst: " << worstall/(numtasks/2) << " (MB/sec)", scope::minimum);
      ...
      tmptimes[1] = avgbw;
      tmptimes[2] = worstbw;
      { scope s(txt() << "Sending timings to task 0");
