...
if (rank == 0) {
   ...
   { scope s("Latency timing test");
	   dbg <<  "Number of reps = " << reps << endl ;
	   dbg << "Task 0 will send " << reps << " times 1 byte messages" << endl <<"to task 1, waiting for a reply between each iteration.";
   }
   dest = 1;
   source = 1;
   for (n = 1; n <= reps; n++) {
	   { scope s(txt() << "Iteration #" << n);
		  T1 = MPI_Wtime();     /* start time */
		  /* send message to worker - message tag set to 1.  */
		  /* If return code indicates error quit */
		  { scope s(txt() << "Send & Recv");
		  ...
		  { scope s(txt() << "Calculating round trip time");
			  dbg << "Round trip time : " << deltaT <<endl;
			  sumT += deltaT;
			  ...
			  dbg << "**Avg round trip time = " << avgT << " microseconds**" << endl ;
			  dbg << "**Avg one way latency = " << avgT/2 << " microseconds**" ;
		  }
	   }
   }
}
