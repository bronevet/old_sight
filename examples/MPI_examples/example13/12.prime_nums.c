...
#define LIMIT     25000     /* Increase this to find more primes */
...
if (rank==0) { scope s(txt()<< "Scan " << LIMIT << " numbers to find PRIMES");}
...
if (rank == FIRST) {
   ...
   { scope s(txt()<<"Start from " << mystart << ", with stride " << stride<< ", scan for prime numbers");}
   { scope s(txt()<<"Send to process 0:");
   { scope s(txt()<< "Number of primes found " << pc, scope::minimum); }
   MPI_Reduce(&pc,&pcsum,1,MPI_INT,MPI_SUM,FIRST,MPI_COMM_WORLD);
   { scope s(txt()<< "The largest prime found " << foundone, scope::minimum); }
