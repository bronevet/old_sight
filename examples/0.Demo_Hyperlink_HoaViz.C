...
int fibScopeLinks(int a, scope::scopeLevel level, list<int>& stack, 
                  ...
                  bool doFWLinks);
int fibGraph(int a, flowgraph& g, anchor* parent);
...
{
  if(argc<2) { printf("Usage: 0.Demo_Hyperlink_HoaViz maxDepth\n"); exit(-1); }
  ...
  string thisFile = txt()<<ROOT_PATH<<"/examples/0.Demo_Hyperlink_HoaViz.C";
  
  SightInit(argc, argv, "Demo_Hyperlink_HoaViz", txt()<<"dbg.0.Demo_Hyperlink_HoaViz.maxDepth_"<<maxDepth);
