...
*/
{
  SightInit(argc, argv, "openMPex7", "dbg.openMPex7.individual");
...
*/
{
  ...
  # pragma omp parallel num_threads(numThreads) shared ( dx, dy, f, itnew, itold, nx, ny, u, unew ) private ( i, it, j )
  {
      ...
      */
      ...
      for ( j = 0; j < ny; j++ )
      {
        for ( i = 0; i < nx; i++ )
        {
          ...
          //scopeOMP s("scope:", forID, nx*ny, j*ny+i);
          //scope s("For1:", scope::minimum);
          //dbg << "Thread " << omp_get_thread_num() <<": u["<<i<<"]["<<j<<"]=" << u[i][j] << endl;  
      ...
      */
      ...
      for ( j = 0; j < ny; j++ )
      {
        for ( i = 0; i < nx; i++ )
        {
          ...
          //scope s("For2:", scope::minimum);
          //dbg << "Thread " << omp_get_thread_num() <<": unew["<<i<<"]["<<j<<"]=" << unew[i][j] << endl;       

          vu = vu + unew[i][j] * unew[i][j];     
        }
        scope s("For2:", scope::minimum);
        dbg << "Thread " << omp_get_thread_num() <<": R8MAT_RMS["<<j<<"]=" << vu << endl; 
      }
    }
    {
